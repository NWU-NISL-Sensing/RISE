...
def start():
    ...
    myclassifier = [svm.SVC(probability = True, break_ties=True, decision_function_shape='ovr', random_state=0),
                ...
                DecisionTreeClassifier(random_state=0),neighbors.KNeighborsClassifier(n_neighbors=4, weights='uniform',algorithm='auto',p=2, 
    ...
    times = ['4'] ##test set
    train_name = ['2','3','1','5'] ##train set
    filepath = r'/root/RISE-Version2/Jupyter/Taprint_A_test/M/data/' 
    filename = ['12_']
    class_index = 3
    class_num =8  
    ...
    data = sio.loadmat(filepath +  filename[0] +  times[0] + 'test.mat')
    xx2 = data['alldata1']
    yy2 = data['alllabel']
    yy2 = yy2.flatten()
    test_x = xx2
    ...
    for ii in train_name:
        data = sio.loadmat(filepath +  filename[0] +  ii+ 'test.mat')
        x1 = data['alldata1']
        y1 = data['alllabel']
        y1 = y1.flatten()
