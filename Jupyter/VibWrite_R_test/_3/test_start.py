...
def start():
    ...
    times = ['3'] ##test set
    train_name = ['11','1','2','4'] ##train set
    filepath = r'/root/RISE-Version2/Jupyter/VibWrite_R_test/_3/data/' 
    filename = ['1014_wyl']
    class_index = 0
    class_num = 10  
    ...
    data = sio.loadmat(filepath + 'difloc_chirp_' + filename[0] + '_fft' + times[0] + '.mat')
    xx2 = data['data_fft']
    yy2 = data['label']
    yy2 = yy2.flatten()
    test_x = xx2
    ...
    yy1 = np.empty(shape=[0,],dtype=int)            
    for ii in train_name:
        data = sio.loadmat(filepath + 'difloc_chirp_' + filename[0] + '_fft' + ii+ '.mat')
        x1 = data['data_fft']
        y1 = data['label']
        y1 = y1.flatten()
